...
<body>
	<!-- 기본 태그로 "Hello" 출력 -->
	<c:out value="Hello" default="안녕"/><br>
	<!-- 기본 태그로 username 이라는 변수를 선언하고 값을 "홍길동"을 준다. 저장영역은 session -->
	<c:set var="username" value="홍길동" scope="session"/><br>
	<!-- 기본 태그로 username을 출력한다. -->
	<c:out value="${sessionScope.username}" default="익명"/><br>
	<c:out value="${username}" default="익명"/><br>
	<c:out value="${null}" default="익명"/><br>
	${username}
	<hr>
	username 값을 슈퍼맨으로 변경<br>
	<!-- username의 값을 슈퍼맨으로 변경. 그리고 username 출력 -->
	<c:set var="username" value="슈퍼맨" scope="session"/>
	${username}
	<hr>
	username 제거
